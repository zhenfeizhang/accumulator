//! This file is part of the accumulator crate.
...
/// hash_to_field use SHA 512 to hash a blob into a non-zero field element
pub(crate) fn hash_to_field<Blob: AsRef<[u8]>>(input: Blob) -> Fr {
    // the hash_to_field_repr_veccom should already produce a valid Fr element
    // so it is safe to unwrap here
    Fr::from_repr(hash_to_field_repr(input.as_ref())).unwrap()
}
